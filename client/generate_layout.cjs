...
const notificationsCode = fs.readFileSync('notifications_code.txt', 'utf8');

const code = `import React, { useState, useEffect } from 'react';
import { Outlet, useLocation, useNavigate, Link } from 'react-router-dom';
import { ChevronLeft, Shield, Timer, X, LayoutDashboard, Library, BookOpen, Calendar, Clock, FileText, GraduationCap, Calculator, User, LogOut, Menu, UserCheck, Flame, Send } from 'lucide-react';
...
${notificationsCode}
  ...
  return (
    <div className="flex h-screen bg-[#f8fafc] overflow-hidden text-slate-900 font-['Inter'] selection:bg-blue-500/30 relative">
      <SEO title={pageTitle} />

      {/* Desktop Sidebar & Mobile Drawer Overlay */}
      {sidebarOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[150] md:hidden" onClick={() => setSidebarOpen(false)} />
      )}

      {/* Sidebar */}
      <aside className={\`fixed md:static inset-y-0 left-0 w-64 bg-white border-r border-slate-200 shadow-2xl md:shadow-none z-[200] transform transition-transform duration-300 ease-in-out flex flex-col \${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}\`}>
        <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-100">
          ...
          <div className="block">
            ...
            <span className="text-[6px] text-blue-500 font-bold uppercase tracking-[0.3em] mt-0.5 block">Official App</span>
          </div>
          <button onClick={() => setSidebarOpen(false)} className="md:hidden ml-auto text-slate-400 hover:text-slate-900">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-4 px-3 space-y-6 custom-scrollbar">
          ...
          <div>
            ...
            <div className="space-y-1">
              ...
              <NavItem to="/notes" icon={BookOpen} label="B.Tech Notes" />
              <NavItem to="/pyq" icon={FileText} label="PYQ Papers" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1">
              <NavItem to="/cgpa" icon={GraduationCap} label="CGPA Calc" />
              <NavItem to="/study" icon={Timer} label="Study Timer" />
              <NavItem to="/calculator" icon={Calculator} label="Calculator" />
              <NavItem to="/ugeac-predictor" icon={Send} label="UGEAC Predictor" />
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="flex items-center justify-between px-4 md:px-8 py-4 bg-white/80 backdrop-blur-xl border-b border-slate-200 z-[100] sticky top-0 shrink-0">
           <div className="flex items-center gap-3">
             <button 
               onClick={() => setSidebarOpen(true)}
               className="p-2 -ml-2 text-slate-600 hover:text-blue-600 bg-slate-50 rounded-xl md:hidden"
             >
               <Menu size={20} />
             </button>
             <span className="text-xs md:text-sm font-[1000] tracking-tighter uppercase text-slate-900 bg-slate-100 px-3 py-1.5 rounded-lg">{pageTitle}</span>
           </div>
           
           {/* Profile Picture Snippet */}
           <div className="flex items-center gap-3">
              <div className="w-8 h-8 md:w-9 md:h-9 bg-slate-100 rounded-xl overflow-hidden shadow-sm border border-slate-200">
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar relative z-10 bg-slate-50/50">
          <div className="w-full h-full pb-24 md:pb-8 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
            <Outlet />
          </div>
        </main>
      </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-xl border-t border-slate-200 pb-safe z-[150] flex items-center justify-around px-2 shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
         ...
         <button onClick={() => setSidebarOpen(true)} className="flex flex-col items-center justify-center gap-1 flex-1 py-2 text-slate-400 hover:text-slate-900">
...
fs.writeFileSync('src/layouts/DashboardLayout.jsx', code);
