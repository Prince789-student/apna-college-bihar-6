...
// 1. Add splitIntoTopics and cleanTitleStr BEFORE parseSyllabusIntoSubjects
const prepCode = `
...
content = content.replace(/function parseSyllabusIntoSubjects/, prepCode);

// 2. Remove hours from explicit unit titles
content = content.replace(/title = trimmed\.replace\(\/\^#\+\\s\*📌\?\\s\*\/\, ''\)\.replace\(\/\^UNIT\\s\+\/i, 'Unit '\)\.trim\(\);/,
  `title = trimmed.replace(/^#+\\s*📌?\\s*/, '').replace(/^UNIT\\s+/i, 'Unit ').trim();
          title = title.replace(/\\s*\\d+\\s*(?:hrs?|hours?)\\s*$/i, '');`);

// 3. Post-process subjects array before returning
const postProcess = `
...
//`;
content = content.replace(/return subjects;\s*}\s*\/\//, postProcess);

// 4. Update TopicRow Component
const topicRowStart = content.indexOf('function TopicRow({ topic, doneKey, subjectName, onToggle }) {');
const topicRowEnd = content.indexOf('function UnitAccordion({ unit, unitIndex, subjectName, semBranchKey, onToggle }) {');
const topicRowNew = `function TopicRow({ topic, doneKey, subjectName, onToggle }) {
...
`;
content = content.substring(0, topicRowStart) + topicRowNew + content.substring(topicRowEnd);
