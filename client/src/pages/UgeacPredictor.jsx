...
function UgeacPredictor() {
  ...
  const downloadRankPredictionPDF = async () => {
    ...
    const jeeCrl = ugeacInput ? parseInt(ugeacInput) : 0;
    const urRank = estimateUgeacRank(jeeCrl);
    ...
    <div className="main-app-container">
      ...
      <>
              ...
              <div className="flex justify-center mb-8">
                 <div className="relative group">
                    ...
                    <div className="bg-slate-900/50 p-2 rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden floating-icon">
                       <img src="/logo-acb.png?v=99" alt="Logo" className="w-20 h-20 rounded-2xl" />
                    </div>
                 </div>
              </div>
              {mode === 'finder' && (
          ...
          <div className="space-y-8 animate-in fade-in max-w-4xl mx-auto">
             <div className="glass-panel bg-white border-slate-200 p-8 rounded-[2.5rem] shadow-xl space-y-6">
                    ...
                    <div className="input-group">
                       <label className="premium-label">JEE Main CRL Rank</label>
                        <input 
                           ...
                           placeholder="Enter JEE Main CRL Rank" 
              ...
              {parseInt(ugeacInput) > 0 && (
                  <div className="glass-panel text-white p-8 rounded-[2.5rem] shadow-2xl space-y-8 animate-in zoom-in-95" style={{ background: '#0f172a' }}>
                     <div className="flex items-center justify-between border-b border-white/10 pb-4">
                        <h4 className="text-xs font-black uppercase tracking-widest text-slate-400">Estimated Rank Report</h4>
                        <div className="flex items-center gap-3">
                           <button 
                              onClick={() => {
                                 const predictedUr = estimateUgeacRank(parseInt(ugeacInput));
                                 const predictedCat = getEstimatedCategoryRank(predictedUr, category);
                                 const predictedRcg = getEstimatedGenderRank(predictedUr, gender);
                                 const predictedCatFemale = getEstimatedCategoryFemaleRank(predictedUr, category, gender);
                                 
                                 setUgeacInput(predictedUr.toString());
                                 if (category !== 'UR') setCatInput(predictedCat.toString());
                                 if (gender === 'Female') setRcgInput(predictedRcg.toString());
                                 if (category !== 'UR' && gender === 'Female') setCatFemaleInput(predictedCatFemale.toString());
                                 
                                 setMode('finder');
                              }}
                              className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-all shadow-md"
                           >
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div className="bg-white/5 border border-white/10 p-6 rounded-2xl text-center space-y-2 relative overflow-hidden group">
                           <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest block">Estimated UGEAC UR Rank</span>
                           <span className="text-4xl font-[1000] text-white block mt-2">
                              #{estimateUgeacRank(parseInt(ugeacInput))}
                           </span>
                           <span className="text-[8px] text-slate-500 font-bold uppercase tracking-wide block">Predicted from JEE CRL</span>
                        </div>
                        ...
                        {category !== 'UR' && (
                           <div className="bg-white/5 border border-white/10 p-6 rounded-2xl text-center space-y-2 relative overflow-hidden group">
                              ...
                              <span className="text-4xl font-[1000] text-emerald-400 block mt-2">
                                 #{getEstimatedCategoryRank(estimateUgeacRank(parseInt(ugeacInput)), category)}
                              </span>
                        ...
                        {gender === 'Female' && (
                           <div className="bg-white/5 border border-white/10 p-6 rounded-2xl text-center space-y-2 relative overflow-hidden group">
                              ...
                              <span className="text-4xl font-[1000] text-pink-400 block mt-2">
                                 #{getEstimatedGenderRank(estimateUgeacRank(parseInt(ugeacInput)), gender)}
                              </span>
                        ...
                        {category !== 'UR' && gender === 'Female' && (
                           <div className="bg-white/5 border border-white/10 p-6 rounded-2xl text-center space-y-2 relative overflow-hidden group">
                              ...
                              <span className="text-4xl font-[1000] text-rose-400 block mt-2">
                                 #{getEstimatedCategoryFemaleRank(estimateUgeacRank(parseInt(ugeacInput)), category, gender)}
                              </span>
                 ...
                 <h2 className="section-title text-indigo-600"><Calculator size={18} /> Candidate Info</h2>
                    ...
                    <div className="input-group">
                       ...
                       <div className="flex justify-between items-center mt-1.5">
                         <span className="text-[8px] text-slate-400 font-bold uppercase tracking-wide">Must be UGEAC State UR Merit Rank</span>
                         <button 
                           type="button"
                           onClick={() => {
                             setUgeacInput('');
                           ...
                           className="text-[9px] font-black text-indigo-600 hover:text-indigo-855 uppercase tracking-widest flex items-center gap-1 transition-all"
                         >
                           <Calculator size={10} /> Predict UGEAC Rank using JEE CRL
                         </button>
                       </div>
                    </div>
        ...
        <footer className="mt-32 py-16 border-t border-white/5 text-center">
           <img src="/logo-acb.png?v=99" alt="Logo" className="w-14 h-14 rounded-full mx-auto mb-8 grayscale opacity-30 hover:opacity-100 transition-opacity" />
      ...
      <div className="bg-white p-8 md:p-12 rounded-[2.5rem] border border-slate-200/50 mt-12 mx-auto max-w-4xl prose prose-slate max-w-none shadow-sm mb-12 text-left">
        <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter mb-4">Ultimate UGEAC College Predictor Tool for BCECEB Counselling</h2>
        <p>
          The <strong>UGEAC College Predictor</strong> is an advanced, data-driven tool designed specifically for Bihar Engineering aspirants. It utilizes official UGEAC cutoff data from previous counseling years (2024, 2025) released by BCECEB to estimate your chances of admission into all 38+ Government Engineering Colleges (GECs) in Bihar, based entirely on your JEE Main AIR (All India Rank).
        </p>

        <h3 className="text-xl font-bold mt-8 mb-4">How to Use the UGEAC Rank Predictor?</h3>
        <ul>
          <li><strong>Step 1:</strong> Enter your JEE Main CRL (Common Rank List) Rank accurately. Do not enter your JEE Category rank here.</li>
          <li><strong>Step 2:</strong> Select your Reservation Category (UR, BC, EBC, SC, ST, EWS, RCG, DQ) exactly as per your official Bihar Domicile certificate.</li>
          <li><strong>Step 3:</strong> Select your Gender. <em>Note: Female candidates in Bihar receive a 33% horizontal reservation under the RCG (Reserve Category Girls) quota, giving them a significant advantage during UGEAC counseling.</em></li>
          <li><strong>Step 4:</strong> Click "Calculate Results". Our proprietary algorithm will convert your JEE Main AIR into an estimated UGEAC State Rank and UGEAC Category Rank to predict your college chances.</li>
        </ul>

        <h3 className="text-xl font-bold mt-8 mb-4">Accuracy of Our UGEAC Predictions</h3>
        <p>
          While our predictor uses highly accurate mathematical interpolation based on the historical UGEAC rank vs. JEE Main rank distribution curves, please note that the exact closing ranks fluctuate every year depending on the total number of UGEAC registrations and the overall difficulty of JEE Mains. We strictly categorize your admission chances into <strong>High, Medium, and Low</strong> to give you a realistic expectation during your choice-filling process.
        </p>
        
        <h3 className="text-xl font-bold mt-8 mb-4">Frequently Asked Questions (FAQs)</h3>
        <p><strong>Q: What is UGEAC?</strong><br/>
        A: UGEAC stands for Undergraduate Engineering Admission Counselling. It is the centralized counseling process conducted annually by the Bihar Combined Entrance Competitive Examination Board (BCECEB) for admission to B.Tech and B.E. courses in Bihar's government and private engineering colleges based on JEE Main scores.</p>
        <p><strong>Q: Is Bihar Domicile necessary for UGEAC Counselling?</strong><br/>
        A: Yes, only candidates who are permanent residents of Bihar or whose parents are government employees posted in Bihar are eligible to claim state quota seats through UGEAC.</p>
        <p><strong>Q: How are choices filled in UGEAC?</strong><br/>
        A: After registration, students must fill their preferred choices of colleges and branches. You can use our "Mock Choice Filling" feature above to simulate this process and download a PDF report of your predicted allotments to help you during the actual BCECEB counseling.</p>
      </div>
    </div>
  );
}
