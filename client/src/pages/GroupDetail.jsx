    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
            <div className="space-y-6 flex-1 text-center md:text-left">
              ...
              <div className="flex flex-wrap items-center gap-3 justify-center md:justify-start">
                {group.meetingLink ? (
                  <>
                    <a href={group.meetingLink.startsWith('http') ? group.meetingLink : `https://${group.meetingLink}`} target="_blank" rel="noopener noreferrer" className="px-6 py-4 md:px-10 md:py-5 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-[1000] text-[10px] md:text-sm uppercase tracking-widest transition-all shadow-2xl shadow-blue-600/30 active:scale-95 flex items-center gap-2 md:gap-3">Enter Hub Link <ExternalLink size={16} /></a>
                    <div className="p-5 bg-white/5 text-slate-500 rounded-2xl border border-white/5 flex items-center gap-2 cursor-not-allowed"><Lock size={20} /><span className="text-[10px] font-black uppercase hidden md:inline">Link Locked</span></div>
                  </>
                ) : (
                  <button onClick={() => setIsSettingLink(true)} className="px-8 py-5 bg-white text-slate-900 rounded-2xl font-[1000] text-[10px] md:text-sm uppercase tracking-widest transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3"><Link2 size={18} className="text-blue-600" /> Attach Meeting Link</button>
                )}
              </div>
            </div>
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
            <div className="bg-white rounded-[3rem] p-10 w-full max-w-lg shadow-2xl space-y-8">
              <div className="text-center space-y-2">
                <div className="w-16 h-16 bg-blue-600/10 text-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-4"><Link2 size={32} /></div>
                <h3 className="text-2xl font-[1000] text-slate-900 uppercase tracking-tighter">Configure Meeting Hub</h3>
                <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Paste your meeting link below.</p>
              </div>
              <div className="space-y-4">
                <input value={newLink} onChange={(e) => setNewLink(e.target.value)} placeholder="https://..." className="w-full bg-slate-100 border-2 border-transparent focus:border-blue-500/50 rounded-2xl p-5 text-slate-900 font-bold outline-none transition-all" />
        ...
        <div className="lg:col-span-2 space-y-8">
           ...
           <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
              {members.length === 0 ? (
                ...
                members.map((member, index) => <MemberCard key={member.id} member={member} index={index} />)
              )}
           </div>
        </div>
        ...
        <div className="space-y-10">
           <div className="bg-slate-900 rounded-[2.5rem] p-8 space-y-8 shadow-2xl relative overflow-hidden border border-slate-800">
              ...
              <div className="flex justify-between items-start">
                 ...
                 {isAdmin && <button onClick={setGroupGoal} className="p-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl transition-all text-white/50 hover:text-white"><Shield size={14} /></button>}
              </div>
              <div className="space-y-4">
                 <div className="flex justify-between items-end"><span className="text-[10px] text-slate-500 font-black uppercase tracking-widest italic">Current Progress</span><span className="text-2xl font-black text-white tracking-tighter">{totalHrs} / {group?.dailyGoal || 1} <span className="text-[10px] text-slate-500">HRS</span></span></div>
                 <div className="h-3 bg-white/5 rounded-full overflow-hidden border border-white/5 p-0.5"><div className="h-full bg-gradient-to-r from-blue-600 to-indigo-500 rounded-full transition-all duration-1000 shadow-[0_0_15px_rgba(37,99,235,0.4)]" style={{ width: `${progress}%` }}></div></div>
              </div>
           </div>
      ...
      {selectedMember && (
        <div className="fixed inset-0 z-[300] flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm transition-all" onClick={() => { setSelectedMember(null); setShowInsights(false); }}>
          <div className="w-full md:w-[400px] bg-white rounded-t-[3rem] md:rounded-[3rem] p-8 shadow-2xl relative transition-all duration-500 animate-in slide-in-from-bottom-full md:slide-in-from-bottom-10" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 mb-8 space-y-6">
              <div className="flex items-center justify-between">
                <div><span className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Today's Focus</span><span className="text-3xl font-[1000] text-slate-900 tracking-tighter">{formatHHMMSS(selectedMember.todayStudyTime)}</span></div>
                <div className="text-right"><span className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Status</span><span className={`text-sm font-black uppercase tracking-widest ${selectedMember.isStudying ? 'text-orange-500' : 'text-slate-400'}`}>{selectedMember.isStudying ? 'Studying' : 'Resting'}</span></div>
              </div>
            </div>
            <div className="flex gap-4">
              <button onClick={() => sendNudge()} className="flex-1 flex items-center justify-center gap-2 py-4 bg-orange-50 text-orange-600 rounded-2xl font-black text-xs uppercase tracking-widest transition-all"><BellRing size={16} /> Nudge</button>
            </div>
          </div>
        </div>
      )}
