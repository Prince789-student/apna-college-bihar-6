    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-5 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-800 pb-10">
        <div className="space-y-4">
           <button onClick={() => navigate('/dashboard/study')} className="flex items-center space-x-2 text-slate-500 hover:text-white transition-colors group">
              <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
              <span className="text-[10px] font-black uppercase tracking-[0.3em]">Return to Network</span>
           </button>
           <div>
              <h1 className="text-4xl font-black text-white tracking-tighter uppercase leading-none">{group.groupName}</h1>
        ...
        <div className="flex flex-col items-center justify-center p-6 bg-[#0d121f] border border-slate-700/50 rounded-3xl shadow-2xl relative group overflow-hidden min-w-[180px]">
           ...
           <span className="text-3xl font-black text-white">{group.memberCount} / {group.maxMembers}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
           <div className="flex items-center space-x-3">
              <div className="p-2 bg-amber-500 rounded-xl shadow-lg shadow-amber-900/20"><Trophy size={20} className="text-white" /></div>
              <h2 className="text-xl font-black text-white uppercase tracking-tighter italic">Daily Leaderboard Rank</h2>
           </div>
           ...
           <div className="space-y-4">
              {members.length === 0 ? (
                ...
                members.map((member, index) => (
                  <div key={member.id} className={`flex items-center justify-between p-6 rounded-[2rem] transition-all relative group shadow-2xl ${index === 0 ? 'bg-gradient-to-r from-blue-600/20 to-indigo-600/10 border-2 border-blue-500/30' : 'bg-[#0d121f] border border-slate-800/80 hover:border-slate-700'}`}>
                     {index === 0 && <div className="absolute -top-3 -right-3 w-10 h-10 bg-amber-500 rounded-full shadow-lg shadow-amber-500/20 flex items-center justify-center border-2 border-[#0d121f] z-10 animate-bounce cursor-default"><Trophy size={16} fill="white" className="text-white" /></div>}
                     
                     <div className="flex items-center space-x-6">
                        <div className={`w-12 h-12 flex items-center justify-center rounded-2xl text-lg font-black italic shadow-lg ${index < 3 ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-500'}`}>
                           #{index + 1}
                        </div>
                        <div>
                           <div className="flex items-center space-x-3">
                              <span className="font-black text-white text-lg tracking-tighter uppercase">{member.name}</span>
                              {member.id === user.uid && <span className="bg-slate-800 text-slate-400 text-[8px] font-black uppercase px-2 py-0.5 rounded-full tracking-widest">YOU</span>}
                           </div>
        ...
        <div className="space-y-10">
           <div className="space-y-6">
              <h3 className="text-sm font-black text-white uppercase tracking-[0.3em] italic flex items-center gap-2">
                 <Clock size={16} className="text-blue-500" /> Operational Specs
              </h3>
              <div className="bg-[#0d121f] border border-slate-800 rounded-3xl p-8 space-y-6 shadow-2xl">
                 ...
                 <div className="flex justify-between items-center border-b border-slate-800/50 pb-4">
                    ...
                    <span className="text-xs font-black text-slate-300 uppercase tracking-widest">50 Seats</span>
                 </div>
                 <div className="flex justify-between items-center border-b border-slate-800/50 pb-4">
           ...
           <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[3rem] p-10 shadow-2xl shadow-blue-900/40 relative overflow-hidden group">
              ...
              <h3 className="text-2xl font-black text-white uppercase tracking-tighter leading-tight mb-8">Ready to climb the rank?</h3>
