    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-white uppercase tracking-tight">Weekly Timetable</h1>
            <p className="text-[11px] text-slate-500">Apna weekly study schedule set karo</p>
          </div>
        </div>
        <button onClick={handleSave} className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all ${saved ? 'bg-emerald-600 text-white' : 'bg-blue-600 hover:bg-blue-500 text-white'}`}>
          <Save size={16} />
          {saved ? '✅ Saved!' : 'Save'}
        </button>
      </div>

      <div className="overflow-x-auto rounded-3xl border border-slate-800/50 bg-[#0d121f]">
        <table className="w-full min-w-[700px]">
          ...
          <tbody>
            {SLOTS.map((slot, si) => (
              <tr key={slot} className={`border-b border-slate-800/30 ${si % 2 === 0 ? '' : 'bg-slate-900/20'}`}>
                  ...
                  return (
                    <td key={key} className="p-1">
                      <input
                        ...
                        placeholder="—"
                        className={`w-full bg-transparent text-center text-xs font-bold text-white outline-none rounded-xl p-2 transition-all placeholder:text-slate-800 hover:bg-slate-800/50 focus:bg-slate-800 focus:ring-1 focus:ring-blue-500/50 ${schedule[key] ? 'bg-blue-600/10 text-blue-300' : ''}`}
                      />
                    </td>
                  );
                })}
