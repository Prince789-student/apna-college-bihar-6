...
export default function Notes() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (uploadData.file) {
        if (uploadData.file.size > 10 * 1024 * 1024) { alert('10MB se badi file nahi chalegi. Drive link use karo!'); setUploading(false); return; }
        const storageRef = ref(storage, `notes/${Date.now()}_${uploadData.file.name.replace(/\s+/g, '_')}`);
      ...
      await addDoc(collection(db, 'documents'), {
        ...
        verified: uploadData.category === 'FOLDER',
        uploadedBy: user?.email || 'Guest'
      });
      setShowUpload(false);
      setUploadData({ title: '', subject: '', category: 'NOTES', semester: '1', file: null, externalUrl: '' });
      alert(uploadData.category === 'FOLDER' ? 'Folder created!' : 'Uploaded! Admin verify karega.');
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-24 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white p-6 md:p-10 rounded-[2.5rem] border border-slate-200/80 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-6">
            <div className="p-3.5 bg-indigo-600/15 text-indigo-500 rounded-2xl">
              <BookOpen size={30} />
            </div>
            <div>
              <h1 className="text-3xl font-[1000] text-slate-900 tracking-tighter uppercase leading-none">Notes Library</h1>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.3em] mt-1">Branch → Semester → Subject → Files</p>
            </div>
          </div>
          ...
          <div className="relative group">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="w-full bg-slate-100 border-2 border-transparent focus:border-indigo-500/40 rounded-2xl py-4 pl-14 pr-5 text-slate-900 text-sm font-bold outline-none transition-all placeholder:text-slate-400" />
          </div>
        </div>
      </div>
              ...
              {subjectFolders.map(f => (
                <button key={f.id} onClick={() => selectFolder(f)}
                  ...
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${branchColor.badge}`}>
                    <FolderOpen size={26} className={branchColor.text.replace('text-', 'text-')} />
                  </div>
                  <div className="flex-1">
                    <h3 className="text-lg font-[900] text-slate-900 uppercase tracking-tight leading-tight">{f.title}</h3>
                    <p className="text-[10px] font-bold text-slate-400 mt-1">{docs.filter(x => x.parentId === f.id).length} files inside</p>
                  </div>
      ...
      {!search.trim() && step === 'files' && (
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <button onClick={() => { setUploadData(u => ({...u, category: 'NOTES'})); setShowUpload(true); }}
              className="ml-auto px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2">
              <FolderPlus size={12} /> Add File
            </button>
          </div>
          {loading ? (
                ...
                <React.Fragment key={d.id}>
                  {d.type === 'folder' ? (
                    <button onClick={() => selectFolder(d)}
                      className="flex flex-col items-start gap-4 p-7 rounded-[2rem] border-2 bg-white hover:bg-indigo-50 border-indigo-100 hover:border-indigo-300 transition-all text-left shadow-sm hover:shadow-md group">
                      <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-indigo-100"><FolderOpen size={22} className="text-indigo-600" /></div>
                      <div>
      ...
      {/* Footer */}
      <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-200/50 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-indigo-100 rounded-xl flex items-center justify-center"><ShieldCheck size={20} className="text-indigo-600"/></div>
          <div>
            <p className="text-slate-900 font-black uppercase text-xs">Verified Academic Content</p>
      ...
      {showUpload && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] border border-slate-200 p-8 space-y-6 animate-in zoom-in-95 duration-300">
            <h2 className="text-xl font-[1000] text-slate-900 uppercase tracking-tighter">Upload Notes</h2>
            <form onSubmit={handleUpload} className="space-y-4">
              <div className="space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Title</p>
                <input type="text" required value={uploadData.title} onChange={e => setUploadData({...uploadData, title: e.target.value})} placeholder="e.g. Unit 1 Notes" className="w-full bg-slate-100 border border-slate-200 rounded-2xl p-4 text-slate-900 text-sm outline-none focus:border-indigo-500" />
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Google Drive Link</p>
                <input type="url" value={uploadData.externalUrl} onChange={e => setUploadData({...uploadData, externalUrl: e.target.value})} placeholder="https://drive.google.com/..." className="w-full bg-slate-100 border border-slate-200 rounded-2xl p-4 text-slate-900 text-sm outline-none focus:border-indigo-500" />
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={uploading} className="flex-1 py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg transition-all">
...
function FileCard({ d, onAction, accentColor = 'indigo' }) {
  ...
  return (
    <div className="bg-white rounded-[2rem] border border-slate-200/80 p-6 hover:border-indigo-300/50 transition-all group flex flex-col shadow-sm hover:shadow-md">
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 ${c.icon} group-hover:scale-110 transition-transform`}>
        <FileDigit size={22} />
      </div>
      <div className="flex-1 space-y-1.5 mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          <span className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border ${c.badge}`}>NOTES</span>
        ...
        {d.subject && <p className="text-[10px] font-bold text-slate-500 flex items-center gap-1"><Bookmark size={9} className="text-indigo-400" />{d.subject}</p>}
      </div>
      <div className="flex items-center gap-2 pt-4 border-t border-slate-100">
        ...
        <button onClick={() => onAction(d.fileUrl, () => { const a = document.createElement('a'); a.href = d.fileUrl; a.download = d.title || 'notes'; document.body.appendChild(a); a.click(); document.body.removeChild(a); })}
          className={`p-3 rounded-xl transition-all border ${c.dl}`}><Download size={13} /></button>
