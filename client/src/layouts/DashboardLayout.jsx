  ...
  const SidebarContent = ({ isMobile = false }) => (
    <>
      <div className={`p-3 md:p-4 flex items-center justify-between ${!isSidebarOpen && !isMobile ? 'justify-center' : ''}`}>
        <div className={`flex items-center gap-2 transition-all duration-300 ${(isSidebarOpen || isMobile) ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'}`}><img src="/logo-acb.png?v=final_fix?v=fresh?v=3" alt="Logo" className="w-8 h-8 rounded-lg" /><div className="min-w-[100px]"><span className="text-sm font-[1000] tracking-tighter uppercase text-slate-900 block leading-none">ACB</span><span className="text-[6px] text-slate-500 font-black uppercase tracking-[0.3em] mt-1 block">Bihar Official</span></div></div>
        ...
        {isStandalone && (
          <div className="flex items-center justify-between px-6 py-4 bg-white/80 backdrop-blur-xl border-b border-slate-200 sticky top-0 z-[100]">
             ...
             <div className="flex items-center gap-3">
                <img src="/logo-acb.png?v=final_fix?v=fresh?v=3" alt="Logo" className="w-8 h-8 rounded-lg" />
