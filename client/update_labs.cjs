...
if (sem2) {
  // Replace the old Python Lab
  sem2.content = sem2.content.replace(/## PYTHON PROGRAMMING LAB[\s\S]*?(?=##|$)/, pythonLabSyllabus);
  // Replace the old IWD Lab
  sem2.content = sem2.content.replace(/## INTRODUCTION TO WEB DESIGN LAB[\s\S]*?(?=##|$)/, iwdLabSyllabus);
}

fs.writeFileSync(file, JSON.stringify(data, null, 2));
console.log('Replaced Lab content');
